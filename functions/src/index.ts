...
async function fetchYouTubeStats(channelId: string, apiKey: string): Promise<{ subscribers: number; views?: number } | null> {
...
async function fetchInstagramStats(_username: string): Promise<{ followers: number } | null> {
...
async function syncSocialStats(): Promise<SocialStats> {
  const [configSnap, existingSnap] = await Promise.all([
    db.doc('mediaKit/config').get(),
    db.doc('mediaKit/socialStats').get(),
  ])
  const config = (configSnap.data() ?? {}) as MediaKitConfig
  const existing = (existingSnap.exists ? existingSnap.data() : {}) as SocialStats
  ...
  if (config.youtubeChannelId && ytKey && ytKey !== 'not-set') {
    ...
    if (youtube) stats.youtube = youtube
  }
  ...
  if (config.tiktokTopVideos && Array.isArray(config.tiktokTopVideos)) {
    stats.tiktokTopVideos = config.tiktokTopVideos.filter((v): v is string => typeof v === 'string').slice(0, 3)
...
export const scheduledSyncSocialStats = onSchedule(
  { schedule: '0 */6 * * *', timeZone: 'Asia/Jerusalem', secrets },
  async () => {
