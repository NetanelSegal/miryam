...
function VotingResultsSection({ voteCounts }: { voteCounts: Record<string, number> }) {
  const entries = Object.entries(voteCounts)
    .map(([id, votes]) => ({ name: candidateNames[id] ?? id, votes }))
    ...
    <div className="w-full max-w-3xl mx-auto px-6">
      ...
      {entries.length > 0 ? (
        <div className="space-y-6">
          {entries.map((entry, i) => (
            <motion.div key={entry.name} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
              transition={{ delay: i * 0.15, duration: 0.5 }} className="flex items-center gap-4">
              <Text size="xl" className="font-bold w-28 md:w-36 text-left shrink-0">{entry.name}</Text>
      ...
      <div className="flex-1 flex items-center justify-center relative z-10">
        <AnimatePresence mode="wait">
          <motion.div key={activeIndex} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            {activeIndex === 0 && <VotingResultsSection voteCounts={voteCounts} />}
