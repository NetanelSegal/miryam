...
import { useCostumeUploadForm } from '@/hooks/forms/useCostumeUploadForm'
...
export function VotingGame() {
  ...
  const costumeForm = useCostumeUploadForm({
    participantId: pid,
      ...
      {!myCostume ? (
        <AnimateOnScroll variant="fade-up" delay={0.1}>
          <Card variant="accent" className="p-6 md:p-8 mb-10 max-w-lg mx-auto">
            ...
            <form onSubmit={costumeForm.handleSubmit} className="space-y-4">
              <Input
                label="שם התחפושת"
                placeholder="למשל: הנסיכה הקסומה"
                {...costumeForm.register('title')}
                error={costumeForm.errors.title?.message}
              />

              <input
                ref={costumeForm.fileInputRef}
                type="file"
                accept="image/*"
                capture="environment"
                className="hidden"
                onChange={costumeForm.handleFileChange}
              />

              {costumeForm.preview ? (
                <div className="relative">
                  <img src={costumeForm.preview} alt="תצוגה מקדימה" className="w-full aspect-[3/4] object-cover" />
                  <button
                    type="button"
                    onClick={costumeForm.clearFile}
                ...
                <button
                  type="button"
                  onClick={() => costumeForm.fileInputRef.current?.click()}
              ...
              <Button
                ...
                type="submit"
                className="w-full"
                disabled={!costumeForm.selectedFile || costumeForm.isSubmitting}
                loading={costumeForm.isSubmitting}
                icon={<Upload className="w-4 h-4" />}
              >
                {costumeForm.isSubmitting ? 'מעלה...' : 'העלאת תחפושת'}
              </Button>
            </form>
          </Card>
        </AnimateOnScroll>
