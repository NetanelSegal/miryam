...
- **Image Upload Tests**: `npm run test:run` — covers `compressImage`, `uploadBlessingPhoto` (blessings), `submitCostume` (costumes), `addCaseStudy` (imageUrl), `useFileUpload`. **These are unit tests with mocked Firebase** — they verify logic, not real uploads. For real browser uploads, configure Storage CORS (see `docs/04-Architecture/STORAGE-CORS.md`).
